...
export default function MySharedRoutesScreen({ navigation }) {
  const [routes, setRoutes] = useState([]);
  const [loading, setLoading] = useState(true);
  const isFocused = useIsFocused();

  useEffect(() => {
    if (isFocused) fetchSharedRoutes();
  }, [isFocused]);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>My Shared Routes</Text>
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No shared routes found.</Text>}
        />
      )}
    </View>
  );
